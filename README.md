...
8. **Installs project skills** — copies `.claude/skills/*/` to `~/.claude/skills/` where Claude Code reads them (full directory, not just SKILL.md — preserves supporting docs and examples). Uses cross-platform path detection to correctly identify user home directory on Windows (`$USERPROFILE`), macOS/Linux (`$HOME`), and Git Bash on Windows (Unix-style path conversion). Creates destination directory if missing, verifies each copy succeeded
